...
def extended_mlp(args):
    ...
    layers = []
    layers.extend([Dense(hidden), act[0]])
    for _ in range(nlayers - 1):
        layers.extend([Dense(hidden), act[1]])
...
vget_unlimited = partial(jax.jit, backend="cpu")(
    jax.vmap(partial(get_trajectory_analytic), (0, None), 0)
)
...
def make_loss(args):
    if args.loss == "l1":
        ...
        def gln_loss(params, batch, l2reg):
            ...
            return (
                jnp.sum(jnp.square(preds - targets)) + l2reg * l2_norm / args.batch_size
            )
...
def train(args, model, data, rng):
    ...
    opt_init, opt_update, get_params = optimizers.adam(
        lambda t: jnp.select(
            [t < args.num_epochs // 2, t >= args.num_epochs // 2], [args.lr, args.lr2]
        )
    )
